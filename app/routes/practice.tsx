...
import type { TestCase, ExpectedAction, TradingAction, NotificationData } from "~/types";
...
export default function Practice() {
  ...
  const startTest = (testCase: TestCase) => {
    ...
    const maxTiming = Math.max(...testCase.expectedActions.map(a => a.timing || 30000));
  ...
  const endTest = () => {
    ...
    // Calculate score
    const results = calculateScore(currentTest, userActions, Date.now() - startTime);
    setTestResults(results);
    setScore(results.totalScore);
  };

  const calculateScore = (testCase: TestCase, actions: TradingAction[], totalTime: number) => {
    ...
    testCase.expectedActions.forEach(expected => {
      ...
      if (matchingActions.length > 0) {
        ...
        const actionTime = new Date(matchingActions[0].timestamp).getTime() - startTime;
  ...
  const filteredTestCases = testCases.filter(test => test.level === selectedLevel);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-800 p-6 rounded-lg">
              <h2 className="text-xl font-semibold mb-4">
                {selectedLevel.charAt(0).toUpperCase() + selectedLevel.slice(1)} Tests
              </h2>
              <div className="grid gap-4">
                {filteredTestCases.map(testCase => (
                  <div key={testCase.id} className="bg-gray-700 p-4 rounded-lg">
                    ...
                    <div className="text-sm">
                      <p><strong>Message:</strong> {testCase.messages[0].title || testCase.messages[0].content}</p>
                      <p><strong>Tickers:</strong> {testCase.messages[0].tickers.join(", ")}</p>
                      <p><strong>Time Limit:</strong> {Math.max(...testCase.expectedActions.map(a => a.timing || 30000)) / 1000}s</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-800 p-4 rounded-lg">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-xl font-semibold">Test: {currentTest?.id}</h2>
                  <p className="text-gray-400">Actions completed: {userActions.length}</p>
                </div>
                <button
                  onClick={endTest}
            ...
            {/* User Actions Log */}
            <div className="bg-gray-800 p-4 rounded-lg">
              <h3 className="text-lg font-semibold mb-3">Your Actions</h3>
              {userActions.length > 0 ? (
                <div className="space-y-2">
                  {userActions.map((action, index) => (
                    <div key={index} className="bg-gray-700 p-2 rounded text-sm">
                      <span className={`font-semibold ${action.action === 'buy' ? 'text-green-400' : 'text-red-400'}`}>
                        {action.action.toUpperCase()}
                      </span>
                      {' '}
                      {action.quantity}x {action.shares} shares of {action.ticker}
          ...
          <div className="space-y-6">
            <div className="bg-gray-800 p-6 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-green-400">{testResults?.accuracyScore}</div>
                  <div className="text-gray-400">Accuracy Score</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-400">{testResults?.speedBonus}</div>
              ...
              <div className="mb-6">
                <p className="text-lg">
                  <strong>Accuracy:</strong> {testResults?.correctActions}/{testResults?.totalExpected} actions correct
                </p>
                <p className="text-lg">
                  <strong>Time:</strong> {Math.round(testResults?.totalTime / 1000)}s
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                {testResults?.details.map((detail: any, index: number) => (
                  <div key={index} className={`p-3 rounded ${detail.completed ? 'bg-green-900' : 'bg-red-900'}`}>
                    <div className="flex justify-between items-center">
                      <span>
                        {detail.expected.action.toUpperCase()} {detail.expected.quantity}x {detail.expected.shares} {detail.expected.ticker}
                      </span>
                      <span className={`font-semibold ${detail.completed ? 'text-green-400' : 'text-red-400'}`}>
