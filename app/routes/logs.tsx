...
export default function Logs() {
  ...
  // Filter states
  const [filterAction, setFilterAction] = useState<string>(searchParams.get("action") || "all");
  const [filterTicker, setFilterTicker] = useState<string>(searchParams.get("ticker") || "");
  ...
  // Export logs to CSV
  const exportToCSV = () => {
    const headers = ["Timestamp", "Action", "Ticker", "Shares", "Quantity", "Success", "Error"];
    const csvContent = [
      headers.join(","),
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-gray-800 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-green-500">{stats.buyTrades}</div>
            <div className="text-sm text-gray-400">Buy Orders</div>
          </div>
          <div className="bg-gray-800 p-4 rounded-lg text-center">
            <div className="text-2xl font-bold text-red-500">{stats.sellTrades}</div>
        ...
        <div className="bg-gray-800 p-4 rounded-lg">
          ...
          {filteredLogs.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-400">
              ...
              <p className="text-sm">Try adjusting your filters or start trading to see logs here</p>
            </div>
          )}
        </div>
      </main>
